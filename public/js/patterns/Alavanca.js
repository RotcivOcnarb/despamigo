...
export default class Alavanca extends BulletPattern {
    ...
    reset() {
        ...
        for (var i = 0; i < 5; i++) {
            var position = new Vector2(this.randomRange(this.game.screen_size.x * 0.4, this.game.screen_size.x * 0.6), this.randomRange(this.game.screen_size.y * 0.1, this.game.screen_size.y * 0.3));
    ...
    update(delta) {
        ...
        var alpha = 0;
        if (this.time < 1)
            alpha = (this.time - .5) * 10;
        else
            alpha = 1 - (this.time - 4.5) * 10;
        ...
        for (var i = 0; i < this.folhas.length; i++) {
            ...
            this.folhas[i].image.setPosition(this.folhas[i].position.x, this.folhas[i].position.y);
        }
