...
var MenuType;
(function (MenuType) {
    MenuType[MenuType["Unselected"] = 0] = "Unselected";
    MenuType[MenuType["Fight"] = 1] = "Fight";
    MenuType[MenuType["Act"] = 2] = "Act";
    MenuType[MenuType["Item"] = 3] = "Item";
    MenuType[MenuType["Mercy"] = 4] = "Mercy";
    MenuType[MenuType["PreBattle"] = 5] = "PreBattle";
    MenuType[MenuType["Battle"] = 6] = "Battle";
})(MenuType || (MenuType = {}));
export default class AmogusScene extends Phaser.Scene {
    constructor() {
        ...
        this.textOptions = []; //Objetos de texto (não as strings)
        this.stringOptions = []; //strings dos objetos de texto
        this.menuType = MenuType.Unselected;
        this.bulletPatterns = [];
        this.bullets = [];
        this.screen_size = new Vector2(window.innerWidth, window.innerHeight);
        this.origin = new Vector2(this.screen_size.x / 2, this.screen_size.y * 0.25);
        this.menuRect = {
        ...
        this.statPosition = new Vector2(this.screen_size.x * 0.2, this.screen_size.y - 220);
    ...
    overlapPointPixelPerfect(point, image) {
        if (!image)
            return false;
    ...
    update(timestep, dt) {
        ...
        if (Math.abs(this.targetRect.x - this.textRect.x) < 20)
            this.textRect.x = this.targetRect.x;
        var dx = Math.sign(this.targetRect.x - this.textRect.x);
        this.textRect.x += dx * dt * 0.5 * 2;
        if (Math.abs(this.targetRect.y - this.textRect.y) < 20)
            this.textRect.y = this.targetRect.y;
        var dy = Math.sign(this.targetRect.y - this.textRect.y);
        this.textRect.y += dy * dt * 0.5 * 2;
        if (Math.abs(this.targetRect.width - this.textRect.width) < 20)
            this.textRect.width = this.targetRect.width;
        var dw = Math.sign(this.targetRect.width - this.textRect.width);
        this.textRect.width += dw * dt * 2;
        if (Math.abs(this.targetRect.height - this.textRect.height) < 20)
            this.textRect.height = this.targetRect.height;
    ...
    setPreBattle(message, bubble_message) {
    ...
    enterBattle() {
        ...
        this.heartPosition = new Vector2(this.targetRect.x + this.targetRect.width / 2, this.targetRect.y + this.targetRect.height / 2);
